...
if [[ -z "$URL" ]];then
  if [[ -z "$default_channel" ]];then
   ...
   wget "$urll/$id/about" -O "$sobre" 2>/dev/null
   ...
   views=$(grep 'watch-view-count' "$vcsabia" | sed 's/<[^>]*>//g;s/ //g;s/Au.*//g')
   criado=$(grep -n 'beigetreten' "$sobre" | sed 's/ //g;s/bei.*//g;s/.*Am//g')
   token=$(grep 'COMMENTS' "$vcsabia" | sed 's/.*: \"//g;s/\".*//g')
   wget "$URL&lc=$token" -O "$codigo" 2>/dev/null
   comentarios=$(grep '<b>Kommentare</b>' "$codigo" | sed 's/.*•//g;s/<[^>]*>//g;s/ //g')
   dados=("$titulo" "$publi" "$inscritos" "$canal" "$likes" "$dislikes" "$views" "$criado" "$comentarios")

}

pega_tudo() {

   echo -e "${cor1}TITULO DO VIDEO: ${cor2}${dados[0]}"
   echo -e "${cor1}DATA DE PUBLICAÇAO DO VIDEO: ${cor2}${dados[1]}"
   echo -e "${cor1}NUMERO DE INSCRITOS: ${cor2}${dados[2]}"
   echo -e "${cor1}NOME DO CANAL: ${cor2}${dados[3]}"
   echo -e "${cor1}TODOS OS LIKES: ${cor2}${dados[4]}"
   echo -e "${cor1}TODOS OS DISLIKES: ${cor2}${dados[5]}"
   echo -e "${cor1}NUMERO DE VISUALIZAÇOES: ${cor2}${dados[6]}"
   echo -e "${cor1}DATA DE CRIAÇAO DO CANAL: ${cor2}${dados[7]}"
   echo -e "${cor1}NUMERO DE COMENTÁRIOS: ${cor2}${dados[8]}"
}
...
while getopts ':abcdefghi' flag; do

  case $flag in
     a) echo -e "${cor1}TITULO DO VIDEO: ${cor2}${dados[0]}";;
     b) echo -e "${cor1}PUBLICAÇAO: ${cor2}${dados[1]}";;
     c) echo -e "${cor1}NUMERO DE INSCRITOS: ${cor2}${dados[2]}";;
     ...
     g) echo -e "${cor1}NUMERO DE VISUALIZAÇOES: ${cor2}${dados[6]}";;
     h) echo -e "${cor1}DATA DE CRIAÇAO DO CANAL: ${cor2}${dados[7]}";;
     i) echo -e "${cor1}NUMERO DE COMENTÁRIOS: ${cor2}${dados[8]}";;
